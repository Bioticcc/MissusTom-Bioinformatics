...
| Raw read QC | FastQC in batches/retries, output-pair checks, then MultiQC. | `FASTQC_RAW` and `MULTIQC_RAW`. | Implemented for included manifest samples and lanes with FastQC 0.12.1 and MultiQC 1.33. |
...
| Clean read QC | FastQC/MultiQC on trimmed FASTQs. | `FASTQC_CLEAN` and `MULTIQC_CLEAN`. | Implemented with declared HTML, ZIP, report, and data-directory outputs. |
...
| Kallisto quant | Paired `--rf-stranded`; complete output means three nonempty files. | `KALLISTO_QUANT`, one task per confirmed sample. | Implemented with Kallisto 0.52.0; each task publishes `abundance.h5`, `abundance.tsv`, and `run_info.json`. |
